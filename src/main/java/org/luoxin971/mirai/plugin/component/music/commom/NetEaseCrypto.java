 ...
 */
package org.luoxin971.mirai.plugin.component.music.commom;
 ...
 * 网易加密工具类
 *
 * @author khjxiaogu file: NetEaseCrypto.java time: 2020年8月26日
 */
public final class NetEaseCrypto {
  ...
  private static String fillString(String str, int size) {
    ...
    while (sb.length() < size) sb.insert(0, "0");
    return sb.toString();
  }
  ...
  public static String[] weapiEncrypt(String content) {
    ...
    String encText =
        NetEaseCrypto.aesEncrypt(
            NetEaseCrypto.aesEncrypt(content, NetEaseCrypto.presetKey, NetEaseCrypto.iv),
            key,
            NetEaseCrypto.iv);
    String encSecKey =
        NetEaseCrypto.rsaEncrypt(key, NetEaseCrypto.publicKey, NetEaseCrypto.modulus);
    try {
