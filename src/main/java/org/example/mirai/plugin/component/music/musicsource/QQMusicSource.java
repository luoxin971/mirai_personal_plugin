 ...
 */
package org.example.mirai.plugin.component.music.musicsource;

import com.google.gson.JsonArray;
import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import org.example.mirai.plugin.component.music.commom.*;
...
public class QQMusicSource implements MusicSource {
  ...
  public String queryRealUrl(String songmid) {
    try {
      JsonObject out =
          HttpRequestBuilder.create("u.y.qq.com")
              .url(
                  "/cgi-bin/musicu.fcg?format=json&data=%7B%22req_0%22%3A%7B%22module%22%3A%22vkey.GetVkeyServer%22%2C%22method%22%3A%22CgiGetVkey%22%2C%22param%22%3A%7B%22guid%22%3A%22358840384%22%2C%22songmid%22%3A%5B%22")
              .url(songmid)
              .url(
  ...
  public String fetchMid(String songid) {
    try {
      JsonObject out =
          HttpRequestBuilder.create("c.y.qq.com")
  ...
  public JsonObject querySongDetail(String songmid) {
    try {
      JsonObject out =
          HttpRequestBuilder.create("u.y.qq.com")
              .url(
                  "/cgi-bin/musicu.fcg?format=json&data=%7B%22req_0%22%3A%7B%22module%22%3A%22music.pf_song_detail_svr%22%2C%22method%22%3A%22get_song_detail_yqq%22%2C%22param%22%3A%7B%22song_mid%22%3A%22")
              .url(songmid)
              .url(
                  "%22%7D%7D%2C%22comm%22%3A%7B%22uin%22%3A%221905222%22%2C%22format%22%3A%22json%22%2C%22ct%22%3A24%2C%22cv%22%3A0%7D%7D")
              .defUA()
              .get()
              .readJson();
      if (out.get("code").getAsInt() != 0) {
  ...
  public MusicInfo get(String keyword) throws Exception {
    ...
    if (musicURL == null) throw new FileNotFoundException();

    return new MusicInfo(
        song.get("title").getAsString(),
        desc,
        "http://y.gtimg.cn/music/photo_new/T002R300x300M000"
            + song.get("album").getAsJsonObject().get("mid").getAsString()
            + ".jpg",
        musicURL,
        "https://i.y.qq.com/v8/playsong.html?_wv=1&songid="
            + song.get("id").getAsString()
  ...
  public MusicInfo getId(String id) throws Exception {
    ...
    try {
      ...
      if (id.length() > 11) throw new RuntimeException();
      mid = fetchMid(id);
    ...
    return new MusicInfo(
        ...
        "http://y.gtimg.cn/music/photo_new/T002R300x300M000"
            + song.get("album").getAsJsonObject().get("mid").getAsString()
            + ".jpg",
        queryRealUrl(song.get("mid").getAsString()),
        "https://i.y.qq.com/v8/playsong.html?_wv=1&songid="
            + song.get("id").getAsString()
            + "&source=qqshare&ADTAG=qqshare",
        "QQ音乐",
